...
static void slot2RtcConfigureBus()
{
    ...
    // Match the bus setup used by checkSlot2/SaveSlot2: the ARM9 must own the
    // GBA slot and the ROM wait states must be relaxed enough for cart chips
    // to respond. The SRAM wait-state bits are intentionally left untouched,
...
{
    if (!gSlot2Active)
        return; // SD mode: keep the original behavior (ROM writes ignored).
